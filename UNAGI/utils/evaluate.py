...
def neighbor_score(adata, embeddingA, type_label):
    ...
    neighbor_label = adata.obs.iloc[np.concatenate(
        embedding_A, axis=0)][type_label]
    cell_label = adata.obs.iloc[np.repeat(np.arange(len(embedding_A)), [
                                          len(j) for j in embedding_A])][type_label]
    n_shared_labels = (neighbor_label.values ==
                       cell_label.values).sum() / len(embedding_A)
...
def run_metrics_subsample(
    ...
    for i, stage in enumerate(stage_keys):
        ...
        adata = adatas[adatas.obs[adatas.obs[stage_key]
                                  == stage].index.tolist()]
        sc.pp.subsample(
            adata,
            fraction=portion,
        ...
        nmi = normalized_mutual_info_score(
            adata.obs['name.simple'], adata.obs['UNAGI'])
        ...
        isolated_asw = scib.metrics.isolated_labels_asw(
            adata, label_key='name.simple', batch_key='stage', embed='z')
        celltype_asw = silhouette_score(
            adata.obsm['z'], adata.obs['name.simple'])
        isolated_f1 = scib.metrics.isolated_labels_f1(
            adata, label_key='name.simple', batch_key='stage', embed='z')
        clisi_graph = scib.metrics.clisi_graph(
            adata,
            type_='embed',
            label_key='name.simple',
            batch_key='stage',
            use_rep='z')
        print('ARI: ', ari)
        print('NMIs: ', nmi)
        ...
        overall_scibs += (clisi_graphs + isolated_asws +
                          ari + nmi + celltype_asw + isolated_f1) / 6
    ...
    return aris / total_stage, NMI / total_stage, DBI / total_stage, label_scores / total_stage, silhouettes / total_stage, isolated_asws / \
        total_stage, cell_type_asws / total_stage, isolated_labels_f1s / total_stage, clisi_graphs / total_stage, overall_scibs / total_stage
