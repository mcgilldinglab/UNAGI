...
class H5ADDataSet(Dataset):
    '''
    The customized dataset for the data without gene weights.
    (For the initial iteration) The dataset will return the gene 
    expression and the cell graph for each cell.
    '''
    ...
    def __getitem__(self, idx):
        ...
        # ,self.data.obs['Sample.ID'][idx]
        return x_tensor, self.data.obsp['gcn_connectivities'][idx].indices.tolist(
        ), idx
...
class H5ADataSetGeneWeight(Dataset):
    '''
    The customized dataset for the data with gene weights.
    The dataset will return the gene expression and the gene 
