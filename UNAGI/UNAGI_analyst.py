...
import subprocess
import numpy as np
from .utils.analysis_helper import find_overlap_and_assign_direction, \
    calculateDataPathwayOverlapGene, calculateTopPathwayGeneRanking, \
        process_customized_drug_database
...
class analyst:
    ...
    def __init__(
        ...
        if target_dir is None:
            self.target_dir = './' + \
                self.data_folder.split('/')[-3] + '_' + str(self.iteration)
            ...
            train_params = json.load(
                open(
                    os.path.join(
                        ...
                        'model_save/training_parameters.json'),
                    'r'))
        else:
            train_params = json.load(
                open(
                    os.path.join(
                        ...
                        'model_save/training_parameters.json'),
                    'r'))
        self.model_name = train_params['task'] + \
            '_' + str(self.iteration) + '.pth'

    def perturbation_analyse_customized_pathway(
            self,
            customized_pathway,
        ...
        '''
        self.adata = calculateDataPathwayOverlapGene(
            self.adata, customized_pathway=customized_pathway)
    ...
    def perturbation_analyse_customized_drug(
            self,
            customized_drug,
            perturbed_tracks='all',
            overall_perturbation_analysis=True,
        ...
        self.adata = process_customized_drug_database(
            self.adata, customized_drug=customized_drug)
        print('Start perturbation....')
        gc.collect()
        a = perturbation(
            self.adata,
    ...
    def start_analyse(
            self,
            progressionmarker_background_sampling,
            run_pertubration,
            random_times,
        ...
        if not os.path.exists(os.path.join(self.target_dir, 'idrem')):
            initalcommand = 'cp -r ' + os.path.join(
                os.path.dirname(
                    self.data_folder),
        ...
        if not ignore_dynamic_markers:
            ...
            if os.path.exists(
                os.path.join(
                    self.target_dir,
                    str(progressionmarker_background_sampling) +
                    'progressionmarker_background.npy')):
                progressionmarker_background = np.load(
                    os.path.join(
                        self.target_dir,
                        str(progressionmarker_background_sampling) +
                        'progressionmarker_background.npy'),
                    allow_pickle=True)
                progressionmarker_background = dict(
                    progressionmarker_background.tolist())
            else:
            ...
            self.adata.uns['progressionMarkers'] = runGetProgressionMarker_one_dist(
                os.path.join(
                    os.path.dirname(
                        self.data_folder),
                    'idremResults'),
        ...
        if not centroid:
            perturbation_runner = perturbation(
                self.adata,
                self.target_dir +
                '/model_save/' +
                self.model_name,
            ...
            perturbation_runner = perturbation_centroid(
                self.adata,
                self.target_dir +
                '/model_save/' +
                self.model_name,
        ...
        if run_pertubration:
            ...
            if not ignore_drug_perturabtion:
                try:
                    temp_drug = np.load(
                        self.customized_drug, allow_pickle=True).item()
                ...
                if self.customized_drug is not None:
                    self.adata = find_overlap_and_assign_direction(
                        self.adata,
                        customized_drug=self.customized_drug,
                    ...
                    if self.cmap_dir is not None:
                        ...
                        raise ValueError(
                            'Please provide a cmap_dir or a customized drug database.')
            print('Start perturbation....')
            gc.collect()
            if not ignore_pathway_perturabtion:
                perturbation_runner.run(
                    'pathway',
                    defulat_perturb_change,
                    inplace=True,
                    CUDA=True)
                print('pathway perturbatnion done')
                print('building random background for pathways....')
                perturbation_runner.run(
                    'random_pathway_background',
                    defulat_perturb_change,
                    inplace=True,
                    CUDA=True)
                perturbation_runner.analysis(
                    'pathway',
                    defulat_perturb_change,
                    perturbed_tracks,
                    overall_perturbation_analysis)
            ...
            if not ignore_drug_perturabtion:
                perturbation_runner.run(
                    'drug', defulat_perturb_change, inplace=True)
                print('drug perturabtion done')

                if self.customized_drug is not None:
                    perturbation_runner.run(
                        'random_drug_background',
                        defulat_perturb_change,
                        inplace=True,
                        random_times=random_times,
                    ...
                    random_gene = self.cmap_overlapped_genes(
                        perturbation_runner.adata.uns['data_drug_overlap_genes'])
                    random_genes = []
                    import random
                    ...
                    for time in range(progressionmarker_background_sampling):
                        ...
                        choices = [random.choice(['+', '-'])
                                   for _ in range(len(random_gene))]
                        random_genes.append(
                            [random_gene[i] + ':' + choices[i] for i in range(len(random_gene))])
        ...
        perturbation_runner.adata.obs['leiden'] = perturbation_runner.adata.obs['leiden'].astype(
            str)
        perturbation_runner.adata.obs['stage'] = perturbation_runner.adata.obs['stage'].astype(
            str)
        perturbation_runner.adata.obs['ident'] = perturbation_runner.adata.obs['ident'].astype(
            str)
        perturbation_runner.adata.write(
            self.target_dir + '/dataset.h5ad',
            compression='gzip',
            compression_opts=9)
        # return a.adata
