...
def calculateTopPathwayGeneRanking(adata):
    ...
    for i in range(1, 4):
        ...
        for clusterid in set(stageadata.obs['leiden']):
            ...
            avg_cluster_gene_weight_table = np.mean(
                cluster_gene_weight_table, axis=0).reshape(-1)

            avg_geneWeightTable_ranking = scipy.stats.rankdata(
                avg_cluster_gene_weight_table)
            avg_geneWeightTable_ranking = np.zeros_like(
                avg_geneWeightTable_ranking) + len(adata_genes) - avg_geneWeightTable_ranking
            ...
            for pathway in list(pathway_gene.keys()):
                ...
                avg_ranking[i][clusterid][pathway] = avg_ranking[i][clusterid][pathway] / \
    ...
    for i in list(avg_ranking.keys()):
        ...
        for j in list(avg_ranking[i].keys()):
            ...
            new_av_ranking[str(i)][str(j)] = {k: idx + 1 for idx, (k, v) in enumerate(
                sorted(avg_ranking[i][j].items(), key=lambda item: item[1]))}
    ...
    for i in range(1, 4):
        ...
        for clusterid in set(stageadata.obs['leiden']):
            ...
            for idx, eachgene in enumerate(avg_cluster_gene_weight_table):
                ...
                if cdf < pval:
                    topClusterGeneBasedOnGeneWeight[i][clusterid].append(
                        adata_genes[idx])
    adata.uns['topClusterGeneBasedOnGeneWeight'] = topClusterGeneBasedOnGeneWeight
    return adata


def merge_drugs_with_sametarget_samedirection(
        adata, overlapped_drug_direction_profile):
    reverse = {}
...
def merge_drugs_with_sametarget_samedirection(
        adata, overlapped_drug_direction_profile=None):
    reverse = {}
    for key in list(overlapped_drug_direction_profile.keys()):
        temp = ''
        for value in overlapped_drug_direction_profile[key]:
...
def assign_drug_direction(adata, cmap_df=None, customized_drug_direction=None):
    ...
    # assign direciton of drugs to regulate genes based on CMAP direction
    # profile
...
def find_overlap_and_assign_direction(
        adata,
        customized_direction=None,
    ...
    if customized_drug is None and customized_direction is None:
        ...
        if (customized_drug is not None):
        ...
        if (customized_direction is not None):
