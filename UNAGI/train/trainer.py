...
from ..train.customized_elbo import myELBO, graphUpdater
...
class UNAGI_trainer():
    ...
    def train_model(
        ...
        for i, [x, neighbourhoods, idx] in enumerate(train_loader):
            ...
            if self.GCN:
                ...
                neighbourhood = [
                    item for sublist in neighbourhoods for item in sublist]
            ...
            if geneWeights is not None:
                ...
                if neighbourhood is not None:
                    ...
                    loss = vae.loss_function(
                        x[idx, :], mu, dropout_logits, mu_, logvar_, gene_weights=geneWeights1)
                else:
                    mu, dropout_logits, mu_, logvar_, _ = vae(x)
                    loss = vae.loss_function(
                        x, mu, dropout_logits, mu_, logvar_, gene_weights=geneWeights1)
                ...
                if neighbourhood is not None:
                    mu, dropout_logits, mu_, logvar_, _ = vae(x, adj, idx)
                    loss = vae.loss_function(
                        x[idx, :], mu, dropout_logits, mu_, logvar_)
                else:
                    mu, dropout_logits, mu_, logvar_, recons = vae(x)
                    loss = vae.loss_function(
                        x, mu, dropout_logits, mu_, logvar_)
                optimizer_vae.zero_grad()
                loss.backward()
                optimizer_vae.step()
                vae_loss += loss.item()
    ...
    def get_latent_representation(self, adata, iteration, target_dir):
        ...
        cell_loader = DataLoader(
            cell, batch_size=self.batch_size, num_workers=0)
        ...
        for i, [x, neighbourhoods, idx] in enumerate(cell_loader):
            if self.GCN:
                ...
                mu, logvar = self.model.encoder(
                    x.view(-1, num_genes), adj, idx)
            else:
                if self.cuda:
                    x = x.to(self.device)
    ...
    def train(
        ...
        if os.path.exists(os.path.join(target_dir, 'model_save',
                          self.modelName + '_' + str(iteration - 1) + '.pth')):
            vae = self.model
            dis = self.dis_model
            ...
            if os.path.exists(
                ...
                if adversarial:
                    dis.load_state_dict(torch.load(os.path.join(
                        target_dir, 'model_save/' + self.modelName + '_dis_' + str(iteration - 1) + '.pth')))
                vae.load_state_dict(torch.load(os.path.join(
                    target_dir, 'model_save/' + self.modelName + '_' + str(iteration - 1) + '.pth')))
        else:
            vae = self.model
            dis = self.dis_model
        ...
        for epoch in range(epoch_range):
            print(epoch)
            total_epoch_loss_train = self.train_model(
                adata,
                vae,
                dis,
                cell_loader,
                ...
                geneWeights=geneWeights if is_iterative else None,
                use_cuda=self.cuda)
