...
def fit_zero_inflated_lognormal(data):
    ...
    log_data = np.log(non_zero_data + 1e-6)
    ...
    ziln_aic = -2 * zero_inflated_model.llf + \
        2 * len(zero_inflated_model.params)
...
def fit_negative_binomial(data):
    ...
    # Only keep non-negative values for NB
    ...
    r = mean_count**2 / \
        (var_count - mean_count) if var_count > mean_count else 1

    # Negative binomial log-likelihood for model comparison
    nb_aic = -2 * nbinom.logpmf(count_data, r, p).sum() + \
        2 * 2  # Two params (r, p)
...
def fit_zero_inflated_negative_binomial(data):
    model = ZeroInflatedNegativeBinomialP(
        data, sm.add_constant(data), exog_infl=sm.add_constant(data))
