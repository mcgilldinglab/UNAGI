...
class analyst:
    ...
    def start_analyse(self,progressionmarker_background_sampling,run_pertubration,defulat_perturb_change=0.5,overall_perturbation_analysis=True,perturbed_tracks='all'):
        ...
        if run_pertubration:
            ...
            print('drug perturabtion done')
            if self.customized_drug is not None:
                a.run('random_background',defulat_perturb_change,inplace=True,random_genes=self.get_median_random_gene(a.adata.uns['data_drug_overlap_genes']))
            else:
                random_gene = self.cmap_overlapped_genes(a.adata.uns['data_drug_overlap_genes'])
                random_genes = [] 
                import random
                ...
                a.adata.uns['cmap_random_genes'] = random_genes
                a.run('random_background_cmap',defulat_perturb_change,inplace=True,random_genes=random_genes)
                a.adata.uns['random_background_perturbation_deltaD'] = a.adata.uns['random_background_cmap_perturbation_deltaD'].copy()
                del a.adata.uns['random_background_cmap_perturbation_deltaD']
            print('random background done')
