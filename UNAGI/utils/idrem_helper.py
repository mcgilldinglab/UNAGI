...
class IDREMthread(threading.Thread):
    ...
    def run(self):
        ...
        command = 'cd ' + \
            str(self.idrem_dir) + ' && java -Xmx8G -jar idrem.jar -b %s %s' % (self.indir, self.outdir)
        p = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True)
        print(p.stdout.read())


def run_certrain_Idrem(
        path,
        file_name,
        idrem_dir,
        species='Human',
        Minimum_Standard_Deviation=0.01,
        Convergence_Likelihood=0.1,
    ...
    settings_file_path = os.path.join(
        midpath, 'idremsetting', f'{prefix_filename}.txt')
                ...
                if trained and k == 4:
                    ...
                    if species == 'Human':
                        f.write(
                            ...
                            ('TF-gene_Interactions_File',
                             'TFInput/human_encode.txt.gz'))
                        continue
                    elif species == 'Mouse':
                        f.write(
                            '%s\t%s\n' %
                            ('TF-gene_Interactions_File',
                             'TFInput/mouse_predicted.txt.gz'))
                        continue
                elif k == 5:
                    if species == 'Human':
                        f.write('%s\t%s\n' %
                    ...
                    f.write(
                        ...
                        ('Minimum_Absolute_Log_Ratio_Expression',
                         str(Minimum_Absolute_Log_Ratio_Expression)))
                    continue
                elif k == 51:
                    f.write(
                        '%s\t%s\n' %
                        ('Convergence_Likelihood_%',
                         str(Convergence_Likelihood)))
                    continue
                elif k == 52:
                    f.write(
                        '%s\t%s\n' %
                        ('Minimum_Standard_Deviation',
                         str(Minimum_Standard_Deviation)))
                    continue
                elif k != 3:
                    f.write(line)
                else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Test the branches in the IDREM json file')
    parser.add_argument(
        '--path',
        type=str,
        help='the file path of IDREM results')
        ...
        if os.path.isdir(os.path.join(args.path, file_name)):
            ...
            while not False:
                ...
                run_certrain_Idrem(
                    ...
                    idrem_dir,
                    species='Human',
                    Minimum_Standard_Deviation=candidates[count][0],
                    Convergence_Likelihood=candidates[count][1],
                    Minimum_Absolute_Log_Ratio_Expression=candidates[count][2],
                    trained=False)
                count += 1
