...
class UNAGI_runner:
    ...
    def load_stage_data(self):
        ...
        for i in range(self.total_stage):
            if self.iteration != 0:
                adata = sc.read(
                    os.path.join(
                        self.data_path, '%d/stagedata/%d.h5ad' %
                        (self.iteration - 1, i)))
    ...
    def annotate_stage_data(self, adata, stage, CPO):
        ...
        '''
        z_locs, z_scales, cell_embeddings = self.trainer.get_latent_representation(
            adata, self.iteration, self.data_path)
        ...
        z_adj = kneighbors_graph(
            adata.obsm['z'],
            self.neighbor_parameters[stage],
            mode='connectivity',
            include_self=True,
            n_jobs=20)
        adata.obsp['connectivities'] = z_adj
        adata.obsp['distances'] = kneighbors_graph(
            adata.obsm['z'],
            self.neighbor_parameters[stage],
            mode='distance',
            include_self=True,
        ...
        if CPO is False:
            sc.pp.neighbors(
                adata,
                use_rep="z",
                n_neighbors=self.neighbor_parameters[stage],
                method='umap')
        ...
        # add top genes, cell types attributes and get average cluster value
        ...
        adata.write(
            os.path.join(
                self.data_path, str(
                    self.iteration) + '/stagedata/%d.h5ad' %
                stage), compression='gzip', compression_opts=9)
    ...
    def run_CPO(self):
        ...
        for i in range(0, len(self.adata_stages)):
            self.adata_stages[i] = self.annotate_stage_data(
                self.adata_stages[i], i, CPO=True)
        if not self.setup_CPO:
            ...
            self.neighbor_parameters, anchor_index = get_neighbors(
                self.adata_stages, num_cells, anchor_neighbors=15, max_neighbors=35, min_neighbors=10)
            self.resolutions, _ = auto_resolution(
                self.adata_stages, anchor_index, self.neighbor_parameters, 0.8, 1.5)
        else:
            self.neighbor_parameters, anchor_index = get_neighbors(
                self.adata_stages, num_cells, anchor_neighbors=self.anchor_neighbors, max_neighbors=self.max_neighbors, min_neighbors=self.min_neighbors)
            self.resolutions, _ = auto_resolution(
                self.adata_stages, anchor_index, self.neighbor_parameters, self.resolution_min, self.resolution_max)
    ...
    def run_IDREM(self):
        ...
        paths = [each for each in paths.values() if len(each) ==
                 self.total_stage]
        # print(paths)
    ...
    def update_gene_weights_table(self, topN=100):
        ...
        '''
        TFs = getTFs(os.path.join(self.data_path,
                                  str(self.iteration) + '/' + 'idremResults' + '/'),
                     total_stage=self.total_stage)
        np.save('test_TFs.npy', np.array(TFs, dtype=object))
        scope = getTargetGenes(
            os.path.join(
                self.data_path, str(
                    self.iteration) + '/' + 'idremResults' + '/'), topN)
        ...
        if self.species == 'Human':
            p = matchTFandTGWithFoldChange(
                ...
                scope,
                self.averageValues,
                get_data_file_path('human_encode.txt'),
                self.genenames,
                self.total_stage)
            ...
            p = matchTFandTGWithFoldChange(
                ...
                scope,
                self.averageValues,
                get_data_file_path('mouse_predicted.txt'),
                self.genenames,
                self.total_stage)
        ...
        updateLoss = updataGeneTablesWithDecay(
            self.data_path, str(
                self.iteration), p, self.total_stage)
    ...
    def build_iteration_dataset(self):
        ...
        mergeAdata(
            os.path.join(
                self.data_path, str(
                    self.iteration)), total_stages=self.total_stage)
