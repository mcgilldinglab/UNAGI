...
def get_neighbors(
    ...
    distance = temp_adata.obsp['distances'][temp_adata.obsp['distances'].nonzero(
    )].reshape(len(temp_adata), -1)
    distance[:, ::-1].sort()
    ...
    for i in range(len(num_cells)):
        if i != anchor_index:
            ...
            for neighbor in range(max_neighbors):
                search.append(
                    abs(np.mean(distance[:, neighbor]) - avg_anchor_distance))
...
def auto_resolution(stagedata, anchor_index, neighbors, min_res, max_res):
    ...
    anchor_adata.obsp['connectivities'] = kneighbors_graph(
        anchor_adata.obsm['z'],
        neighbors[anchor_index],
        mode='connectivity',
        include_self=True,
        n_jobs=20)
    anchor_adata.obsp['distances'] = kneighbors_graph(
        anchor_adata.obsm['z'],
        neighbors[anchor_index],
        mode='distance',
        include_self=True,
    ...
    for i in range(len(stagedata)):
        ...
        if i != anchor_index:
            temp_adata = stagedata[i]
            temp_adata.obsp['connectivities'] = kneighbors_graph(
                temp_adata.obsm['z'],
                neighbors[i],
                mode='connectivity',
                include_self=True,
                n_jobs=20)
            temp_adata.obsp['distances'] = kneighbors_graph(
                temp_adata.obsm['z'],
                neighbors[i],
                mode='distance',
                include_self=True,
            ...
            for j in np.arange(min_res, max_res + 0.1, 0.1):
                ...
                temp_mean, temp_median = get_mean_median_cell_population(
                    temp_adata)
