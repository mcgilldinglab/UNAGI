        ...
        if 'overall' not in adata.uns['drug_perturbation_score'][str(intensity)].keys():
            ...
            if selected_track == None:
                ...
                if selected_track not in adata.uns['drug_perturbation_score'][str(intensity)].keys():
                    raise ValueError('Not a valid track! Valid tracks are: %s' % (list(adata.uns['drug_perturbation_score'][str(intensity)].keys())))
                else:
...
def get_top_single_genes(adata, intensity, top_n=None, cutoff=None, selected_track=None):
    ...
    if top_n is not None:
        if 'overall' not in adata.uns['single_gene_perturbation_score'][str(intensity)].keys():
            ...
            if selected_track == None:
                ...
                if selected_track not in adata.uns['single_gene_perturbation_score'][str(intensity)].keys():
                    raise ValueError('Not a valid track! Valid tracks are: %s' % (list(adata.uns['single_gene_perturbation_score'][str(intensity)].keys())))
                else:
        ...
        print('Please specify top_n or cutoff')
def get_top_gene_combinations(adata, intensity, top_n=None, cutoff=None, selected_track=None):
    '''
    Get top gene-combination predictions after gene-combination perturbations at a given intensity.
    ...
    if top_n is not None:
        if 'overall' not in adata.uns['two_genes_perturbation_score'][str(intensity)].keys():
            dict_results = {}
            if selected_track == None:
                for each_track in adata.uns['two_genes_perturbation_score'][str(intensity)].keys():
                    if 'top_compounds' not in adata.uns['two_genes_perturbation_score'][str(intensity)][each_track].keys():
                        print('All pertubred genes are not statistically significant in track %s!'%(each_track))
                        dict_results[each_track] = {}
                    else:
                        dict_results[each_track] = pd.DataFrame.from_dict(adata.uns['two_genes_perturbation_score'][str(intensity)][each_track]['top_compounds'])[:top_n]
                print('You are checking the perturbation results for individual tracks, the returning results are stored in the dictionary.\n You can access the results by using the track name as the key.')
                print('Here are the keys for the dictionary:')
                for key in dict_results.keys():
                    print(key)
                ...
                if selected_track not in adata.uns['two_genes_perturbation_score'][str(intensity)].keys():
                    raise ValueError('Not a valid track! Valid tracks are: %s' % (list(adata.uns['two_genes_perturbation_score'][str(intensity)].keys())))
                else:
                    temp = pd.DataFrame.from_dict(adata.uns['two_genes_perturbation_score'][str(intensity)][selected_track]['top_compounds'])
                    temp = temp.sort_values(by='pval_adjusted',ascending=True)
            ...
            if selected_track == None:
                if 'top_compounds' not in adata.uns['two_genes_perturbation_score'][str(intensity)]['overall'].keys():
                    if 'down_compounds' in adata.uns['two_genes_perturbation_score'][str(intensity)]['overall'].keys():
                        print('All pertubred genes are not statistically significant!')
                        print('Here are some top insiginifcant genes, but they are not statistically significant:')
                        return pd.DataFrame.from_dict(adata.uns['two_genes_perturbation_score'][str(intensity)]['overall']['down_compounds'])[:top_n]
                temp = pd.DataFrame.from_dict(adata.uns['two_genes_perturbation_score'][str(intensity)]['overall']['top_compounds'])
                temp = temp.sort_values(by='pval_adjusted',ascending=True)
                temp.rename(columns={'compound': 'gene combination'}, inplace=True)
                if 'drug_regulation' in temp.columns:
                    temp = temp.drop(columns=['drug_regulation'])
                ...
                if selected_track not in adata.uns['two_genes_perturbation_score'][str(intensity)].keys():
                    raise ValueError('Not a valid track! Valid tracks are: %s' % (list(adata.uns['two_genes_perturbation_score'][str(intensity)].keys())))
                if 'top_compounds' not in adata.uns['two_genes_perturbation_score'][str(intensity)][selected_track].keys():
                    if 'down_compounds' in adata.uns['two_genes_perturbation_score'][str(intensity)][selected_track].keys():
                        print('All pertubred compounds are not statistically significant!')
                        print('Here are some top insiginifcant compounds, but they are not statistically significant:')
                        temp = pd.DataFrame.from_dict(adata.uns['two_genes_perturbation_score'][str(intensity)][selected_track]['down_compounds'])
                        temp = temp.sort_values(by='pval_adjusted',ascending=True)
                        temp.rename(columns={'compound': 'gene combination'}, inplace=True)
                        if 'drug_regulation' in temp.columns:
                            temp = temp.drop(columns=['drug_regulation'])
            ...
            if selected_track == None:
                for each_track in adata.uns['two_genes_perturbation_score'][str(intensity)].keys():
                    if 'top_compounds' not in adata.uns['two_genes_perturbation_score'][str(intensity)][each_track].keys():
                        ...
                        temp = pd.DataFrame.from_dict(adata.uns['two_genes_perturbation_score'][str(intensity)][each_track]['top_compounds'])
                        dict_results[each_track] = temp[temp['pval_adjusted'] < cutoff]
                print('You are checking the perturbation results for individual tracks, the returning results are stored in the dictionary.\n You can access the results by using the track name as the key.')
                print('Here are the keys for the dictionary:')
                for key in dict_results.keys():
            ...
            if selected_track == None:
                if 'top_compounds' not in adata.uns['two_genes_perturbation_score'][str(intensity)]['overall'].keys():
                    if 'down_compounds' in adata.uns['two_genes_perturbation_score'][str(intensity)]['overall'].keys():
                        print('All pertubred genes are not statistically significant!')
                        print('Here are some top insiginifcant genes that are not statistically significant:')
                        return pd.DataFrame.from_dict(adata.uns['two_genes_perturbation_score'][str(intensity)]['overall']['down_compounds'])
                temp = pd.DataFrame.from_dict(adata.uns['two_genes_perturbation_score'][str(intensity)]['overall']['top_compounds'])
                temp.rename(columns={'compound': 'gene combination'}, inplace=True)
                if 'drug_regulation' in temp.columns:
                    temp = temp.drop(columns=['drug_regulation'])
                ...
                if 'top_compounds' not in adata.uns['two_genes_perturbation_score'][str(intensity)][selected_track].keys():
                    if 'down_compounds' in adata.uns['two_genes_perturbation_score'][str(intensity)][selected_track].keys():
                        print('All pertubred genes are not statistically significant!')
                        print('Here are some top insiginifcant genes that are not statistically significant:')
                        return pd.DataFrame.from_dict(adata.uns['two_genes_perturbation_score'][str(intensity)][selected_track]['down_compounds'])
                temp = pd.DataFrame.from_dict(adata.uns['two_genes_perturbation_score'][str(intensity)][selected_track]['top_compounds'])
                temp.rename(columns={'compound': 'gene combination'}, inplace=True)
                if 'drug_regulation' in temp.columns:
                    temp = temp.drop(columns=['drug_regulation'])
