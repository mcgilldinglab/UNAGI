...
def graphUpdater(
    ...
    with poutine.trace(param_only=True) as param_capture:
        loss_vae, surrogate_loss_particle, loss_discriminator = loss.loss_and_grads(
            model, guide, discriminator, device, x, adj, i, start, end)
        surrogate_loss_particle.backward()
    params = set(site["value"].unconstrained() for site in param_capture.trace.nodes.values(
    ) if 'discriminator' not in site['name'] and 'recon' not in site['name'])
    optim(params)
    ...
    with poutine.trace(param_only=True) as param_capture:
        loss_vae, surrogate_loss_particle, loss_discriminator = loss.loss_and_grads(
            model, guide, discriminator, device, x, adj, i, start, end)
    params = set(site["value"].unconstrained() for site in param_capture.trace.nodes.values(
    ) if 'discriminator' in site['name'] and 'recon' not in site['name'])
...
class myELBO(ELBO):
    ...
    def get_importance_trace(
        ...
        guide_trace = poutine.trace(
            guide, graph_type=graph_type).get_trace(
            *args, **kwargs)
        if detach:
            guide_trace.detach_()
        ...
        if is_validation_enabled():
            check_model_guide_match(
                model_trace, guide_trace, max_plate_nesting)
        ...
        if self.vectorize_particles:
            if self.max_plate_nesting == float("inf"):
                self._guess_max_plate_nesting(
                    model, guide, discriminator, args, kwargs)
    ...
    def loss_and_grads(
        ...
        for model_trace, guide_trace in self._get_traces(
            ...
            if self.pushback_Score is not None:
                ...
                loss += 0.00001 * (loss_particle) / self.num_particles + \
                    vae_add + 10000 * pushback_Score  # self.pushback_Score
            ...
            if trainable_params and getattr(
                ...
                if self.pushback_Score is not None:
                    surrogate_loss_particle = 0.00001 * surrogate_loss_particle / \
                        self.num_particles + vae_add + 10000 * pushback_Score
                else:
                ...
                if "log_prob" not in site:
                    try:
                        if self.geneWeight is not None and name == 'obs':
                            ...
                            log_p_gene = site["fn"].base_dist.log_prob(
                                site["value"]) * self.geneWeight
                            # 1 is the indicator of event, showed in
                            # model.sample.event
                            log_p = self._sum_rightmost(log_p_gene, 1)
                        ...
                        raise ValueError(
                            "Error while computing log_prob at site '{}':\n{}\n{}".format(
                                name, exc_value, shapes)).with_traceback(traceback) from e
                    site["unscaled_log_prob"] = log_p
                    log_p = scale_and_mask(log_p, site["scale"], site["mask"])
                    site["log_prob"] = log_p
                    site["log_prob_sum"] = log_p.sum()
                    # site["log_prob_sum"] = log_p.mean() #use mean to try to
                    # tackle large batch size nan issue
                    if is_validation_enabled():
                        warn_if_nan(
                            site["log_prob_sum"],
